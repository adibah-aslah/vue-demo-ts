  ...
  <div class="relative w-full" ref="dropdownRef">
    <div
      class="w-full border rounded-lg px-3 py-3 flex items-center flex-wrap gap-1 cursor-pointer
             bg-white dark:bg-gray-800 dark:text-white focus-within:border-blue-500 relative"
    ...
    >
      <template v-if="selectedLabels.length">
        <span
          ...
          :key="label"
          class="bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 text-xs px-2 py-0.5 rounded-md"
        >
          {{ label }}
        </span>
        <span
      ...
      <span v-else class="text-gray-400 text-sm">{{ label }}</span>
